...
use zomato
create table zomato(restaurant varchar(100) not null primary key, 
...
-- Most Popular Rest: top 10 rest. with highest avg rating
...
--Identify the top-rated restaurants that offer online ordering in each city.
...
--Which areas offer the most affordable dining options based on the average cost for two?
...
--Which city offers the most diverse range of cuisines? AND WHAT ARE THOSE CUISINES using Outer query/group concat
...
--City Comparison of Dining Costs:Avg cost for 2 in each city
...
--Hidden gems: less rating count high rating
...
--a)Distributn of rest rating 
--b)Which city has the highest average rating #MUM has 4 avg 
select city,rating,count(*)as rating_count from zomato group by city,rating order by city,rating desc 
select city,round(avg(rating),0)as avg_rat from zomato group by city order by avg_rat desc  
